...
const upload = multer({
  storage: multer.diskStorage({
    ...
    filename: (_req, file, callback) => callback(null, `${Date.now()}-${Math.random().toString(36).slice(2)}-${path.basename(file.originalname)}`)
  }),
...
app.post("/api/upload", requireAdmin, upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const requestedName = String(req.body.filename || req.file.originalname).trim();
    const filename = path.basename(requestedName).replace(/[\\/]/g, "-").trim();
    if (!filename || filename === "." || filename === "..") return res.status(400).json({ ok: false, error: "A valid filename is required." });
    const result = await uploadToGithub(req.body.release_id, filename, req.file.mimetype, temp, req.file.size);
